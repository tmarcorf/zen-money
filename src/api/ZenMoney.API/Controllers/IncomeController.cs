...
namespace ZenMoney.API.Controllers
{
        ...
        IIncomeService incomeService) : ControllerBase
    {
        ...
        public async Task<IActionResult> GetByIdAsync(Guid id)
        {
            ...
            if (!result.IsSuccess)
            {
                return NotFound(ApiResponse<IncomeModel>.Failure(result.Error, "404"));
            }
        ...
        public async Task<IActionResult> ListPaginatedAsync([FromQuery] SearchIncomeRequest request)
        {
            ...
            if (!result.IsSuccess)
            {
                return NotFound(ApiResponse<List<IncomeModel>>.Failure(result.Error, "404"));
            }
        ...
        public async Task<IActionResult> CreateAsync(CreateIncomeRequest request)
        {
            ...
            if (!result.IsSuccess)
            {
                return BadRequest(ApiResponse<IncomeModel>.Failure(result.Error));
            }
